function [idx, C, sumD, D] = kmeans_function(X, k, varargin)
%kmeans_function K-means clustering.
%   IDX = kmeans_function(X, K) partitions the points in the N-by-P data matrix
...
%   variables.  kmeans_function returns an N-by-1 vector IDX containing the
%   cluster indices of each point.  By default, kmeans_function uses squared
%   Euclidean distances.
%
%   [IDX, C] = kmeans_function(X, K) returns the K cluster centroid locations in
%   the K-by-P matrix C.
%
%   [IDX, C, SUMD] = kmeans_function(X, K) returns the within-cluster sums of
%   point-to-centroid distances in the 1-by-K vector sumD.
%
%   [IDX, C, SUMD, D] = kmeans_function(X, K) returns distances from each point
%   to every centroid in the N-by-K matrix D.
%
%   [ ... ] = kmeans_function(..., 'PARAM1',val1, 'PARAM2',val2, ...) allows you to
%   specify optional parameter name/value pairs to control the iterative
%   algorithm used by kmeans_function.  Parameters are:
%
%   'Distance' - Distance measure, in P-dimensional space, that kmeans_function
...
%                              kmeans_function infers K from the first dimension of
...
%       [cidx, ctrs] = kmeans_function(X, 2, 'dist','city', 'rep',5, 'disp','final');
%       plot(X(cidx==1,1),X(cidx==1,2),'r.', ...
%            X(cidx==2,1),X(cidx==2,2),'b.', ctrs(:,1),ctrs(:,2),'kx');
%
%   See also LINKAGE, CLUSTERDATA, SILHOUETTE.

%   kmeans_function uses a two-phase iterative algorithm to minimize the sum of
...
%   kmeans_function can converge to a local optimum, which in this case is a
...
for rep = 1:reps
    ...
    case 'cluster'
        ...
        [dum, C] = kmeans_function(Xsubset, k, varargin{:}, 'start','sample', 'replicates',1);
    case 'numeric'
        C = CC(:,:,rep);
    end    
