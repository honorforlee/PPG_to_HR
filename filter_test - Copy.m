...
function filter_test_OpeningFcn(hObject, ~, h, varargin)
    ...
    h.ft1 = []; h.ft2 = []; h.ft3 = []; h.fs1 = []; h.fs2 = []; h.fs3 = [];
    h.gt1 = []; h.gt2 = []; h.gt3 = []; h.gs1 = []; h.gs2 = []; h.gs3 = [];
    h.output = hObject;
    h = update_infile(h);
...
function h = quantisize_input(h)
    if h.ecg  % TODO
        h.dt = h.dt0; h.t = h.t0; h.s = h.s0;  % TODO
        return;
    end  % TODO
    h.dt   = 1/str2double(h.edit_f_mes.String);
    h.dNds = str2double(h.edit_dNdS.String);
    h.t = h.t0(1):h.dt:h.t0(end);
...
function h = grids(h)
    ...
    if h.checkbox_f_mes.Value
        ...
        h.ygrid = [ h.ygrid  nan  repmat(10*[min(h.s) max(h.s) nan],1,length(h.t)) ];
    end
...
%             )
%     else
        d  = h.s(2:end) - h.s(1:end-1);  td  = ( h.t(2:end) + h.t(1:end-1) ) / 2;
        d2 =   d(2:end) -   d(1:end-1);  td2 = (  td(2:end) +  td(1:end-1) ) / 2;
        [tx,sx, dhi, dlo] = signal_picks(h.t,h.s);
        [ty,sy,d2hi,d2lo] = signal_picks( td,  d);
        xl = h.axes.XLim;
        yl = h.axes.YLim;
        hold off
...
function [tx,sx,dhi,dlo] = signal_picks(t,s)
    d  =  s(2:end) -  s(1:end-1);   td = (  t(2:end) +  t(1:end-1) ) / 2;
    kx = d > 0;
    ...
    dlo = d(kx+1);
    for k = 1:length(kx)
        i = kx(k)-1;   while i > 0         && d(i) >= dhi(k); dhi(k) = d(i); i = i-1; end
