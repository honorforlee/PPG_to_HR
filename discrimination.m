...
range = (1 : (10/h.dt)) * h.dt;             % create subset data of 10s each
for k = 0 : length(h.s) / length(range) - 1
    
    t_div (k+1,:) =  h.t(  k*(length(range)) + 1 : (k+1)*(length(range)) ) ;
    s_div(k+1,:)= h.s(  k*(length(range)) + 1 : (k+1)*(length(range)) ) ;
...
if isempty(h.ft)
    ...
    [h.kx,h.tx,h.sx, h.dhi,h.dlo, h.td, h.d, h.tx_N,h.sx_N, h.note_x] = signal_peaks(t_div(1,:), s_div(1,:));
else
    [h.kx,h.tx,h.sx, h.dhi,h.dlo, h.td, h.d, h.tx_N,h.sx_N, h.note_x] =  signal_peaks(h.ft, h.fs);      %   filter applied before derivative
end
[h.ft_,h.fs_] = apply_filter_( h.td , h.d , h.edit_F2.String );
